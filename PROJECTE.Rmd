...
pie(table(df$isNewConstruction))
...
mcm <- mc1 <- lm(log(price)~log(livingArea)+f.bath +f.bed+ f.antig + pool + levels + event + parking + f.gar + spa + isNewConstruction + homeType + f.sqrtPrice + hasPetsAllowed, data = df)
# AFEGIM CATEGORIQUES. -> SEONGS EL MODEL MÉS BO NUMERIC DE ADALT.
mc1 <- lm(log(price)~log(livingArea)+log(bathrooms)+poly(bedrooms,2)+poly(antiguetat,2) + pool + levels + event + parking + f.gar + spa + isNewConstruction + homeType + f.sqrtPrice + hasPetsAllowed, data = df)
AIC(mcm,mc1)
mc1 <- lm(log(price)~log(livingArea)+f.bath +f.bed+ f.antig + pool + levels + event + parking + f.gar + spa + isNewConstruction + homeType + f.sqrtPrice + hasPetsAllowed, data = df)
summary(mc1)
par(mfrow=c(2,2))
plot(mc1)
...
mi1 <- lm(log(price)~log(livingArea)+f.bath + f.sqrtPrice, data = df)
summary(mi1)
...
stepi <- lm(log(price)~(log(livingArea))*(f.bath + f.sqrtPrice), data = df)
