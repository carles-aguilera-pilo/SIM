...
```{r}
res.con <- condes(df[,c(3:21)], num.var = 5)
res.con$quanti
...
model3 <- lm(df$price~df$livingArea+df$bathrooms+df$bedrooms)
summary(model3)
par(mfrow=c(2,2))
plot(model3)
vif(model3)
...
par(mfrow=c(1,1))
boxCox(model3) # Value close to 0.5, let's apply sqrt transformation.

modelv3 <- lm(sqrt(df$price)~df$livingArea+df$bathrooms+df$bedrooms)
summary(modelv3)
par(mfrow=c(2,2))
plot(modelv3)
boxTidwell(sqrt(df$price)~df$livingArea+df$bathrooms+df$bedrooms)

modelv33 <- lm(sqrt(price) ~ I(livingArea^0.26) + sqrt(bathrooms) + I(1 / (bedrooms^0.3)),data = df)
summary(modelv33)
par(mfrow=c(2,2))
plot(modelv33)
vif(modelv33)

# INTERACTIONS -> NOT BETTER.
modelv333 <- lm(sqrt(price) ~ I(livingArea^0.26) + sqrt(bathrooms)* I(1 / (bedrooms^0.3)),data = df)
summary(modelv333)
vif(modelv333)
modelv333 <- lm(sqrt(price) ~ I(livingArea^0.26)*I(1 / (bedrooms^0.3)) + sqrt(bathrooms),data = df)
summary(modelv333)
vif(modelv333)
```
...
```{r}
model4 <- lm(df$price~df$livingArea+df$bathrooms+df$bedrooms+df$garageSpaces)
summary(model4)
par(mfrow=c(2,2))
plot(model4)
vif(model4)
...
modelv4 <- lm(sqrt(df$price)~df$livingArea+df$bathrooms+df$bedrooms+df$garageSpaces)
summary(modelv4)
par(mfrow=c(2,2))
plot(modelv4)
vif(modelv4)
...
modelv44 <- lm(sqrt(df$price)~log(df$livingArea)+sqrt(df$bathrooms)+log(df$bedrooms)+df$garageSpaces)
summary(modelv44)
par(mfrow=c(2,2))
plot(modelv44)
vif(modelv44) # SEGONS CERTS ANÀLISI D?INTERNET EL MILLOR SERIA AQUEST. (AGAFEM AQUEST DE MOMENT).
residualPlots(modelv44)
stepx <- step(modelv44, k=log(nrow(df)))
...
```{r}
model5 <- lm(sqrt(df$price)~df$livingArea+df$bathrooms+df$bedrooms+df$garageSpaces+df$yearBuilt)
summary(model5)
par(mfrow=c(2,2))
plot(model5)
par(mfrow=c(1,1))
boxCox(model5) # Close to 1, so we will not make any change. WE save the sqrt
vif(model5)
Anova(model5)


model55 <- lm(sqrt(df$price)~log(df$livingArea)+sqrt(df$bathrooms)+log(df$bedrooms)+df$garageSpaces+df$yearBuilt)
summary(model55)
par(mfrow=c(2,2))
plot(model55)
vif(model55)

model55_noYear <- update(model55, . ~ . - df$yearBuilt)
anova(model55_noYear, model55)

model5_ <- lm(sqrt(df$price)~log(df$livingArea)+sqrt(df$bathrooms)+log(df$bedrooms)+df$garageSpaces)
summary(model5_)
vif(model5_)

# ES MILLOR NO INCLOURE EL YEARBUILT, AMB ANOVA HO HEM COMPROVAT. -> MODEL 5_no_year = model_5
...
model5d <- lm(sqrt(df$price)~log(df$livingArea)+sqrt(df$bathrooms)+log(df$bedrooms)+df$garageSpaces+df$f.year)
summary(model5d)

model5d2 <- lm(sqrt(df$price)~log(df$livingArea)+sqrt(df$bathrooms)+log(df$bedrooms)+df$garageSpaces+df$f.antig)
AIC(model5d, model5d2)
vif(model5d)
anova(model5_, model5d) # EL MILLOR model5d
AIC(model5_, model5d)
...
```{r}
m1 <- lm(sqrt(sqrt(df$price))~log(df$livingArea)+sqrt(df$bathrooms)+log(df$bedrooms)+df$garageSpaces+df$f.year+factor(df$levels))
summary(m1)
par(mfrow=c(2,2))
plot(m1)
str(df$f.year)
m2 <- lm(sqrt(sqrt(df$price))~log(df$livingArea)+sqrt(df$bathrooms)+log(df$bedrooms)+df$garageSpaces+df$f.year*factor(df$levels))
summary(m2)
par(mfrow=c(2,2))
plot(m2)
...
m3 <- lm(sqrt(sqrt(df$price))~log(df$livingArea)+sqrt(df$bathrooms)+log(df$bedrooms)+df$garageSpaces+df$f.year*(factor(df$levels) + factor(df$city)+factor(df$county)))
summary(m3)
par(mfrow=c(2,2))
plot(m3)
AIC(m1,m2)
...
```{r}
mp <- lm(sqrt(sqrt(df$price))~log(df$livingArea)+sqrt(df$bathrooms)+log(df$bedrooms)+df$garageSpaces+df$f.year+factor(df$levels) + factor(df$city)+factor(df$county)+factor(df$spa)+factor(df$pool))
summary(mp)
par(mfrow=c(2,2))
plot(mp)
```
...
```{r}
stepm1 <- step(m1, k=log(nrow(df)))
modelstep1 <- lm(sqrt(sqrt(df$price)) ~ log(df$livingArea) + sqrt(df$bathrooms) + 
    log(df$bedrooms) + df$garageSpaces + factor(df$city))
summary(modelstep1)
vif(modelstep1)
```
