...
df$f.sqrtPrice <- cut(df$pricePerSquareFoot, quantile(df$pricePerSquareFoot, probs = seq(0,1,0.2)), include.lowest = TRUE)
summary(df$f.sqrtPrice)
...
res.cat <- catdes(df[,c(3:25)], num.var = 23)
...
```{r}
corr_mat <- cor(df[,c(3,4,6:9,11,22)])
corr_mat
...
```{r}
res.con <- condes(df[,c(2:24)], num.var = 2)
res.con$quanti
res.con$quali
...
mx2 <- lm(log(price)~log(livingArea)+log(bathrooms)+poly(bedrooms,2)+poly(antiguetat,2), data = df)
summary(mx2)
par(mfrow=c(2,2))
plot(mx2)
residualPlots(mx2)
...
AIC(mx1,mx2) 
"AQUI FINS LA PRIMERA VERSIO"
# MILLLOR MX2.

# AFEGIM CATEGORIQUES. -> SEONGS EL MODEL MÉS BO NUMERIC DE ADALT.
mc1 <- lm(log(price)~log(livingArea)+log(bathrooms)+log(bedrooms)+poly(antiguetat,2) + pool + levels + event + parking + f.gar + spa + isNewConstruction + homeType + f.sqrtPrice + hasPetsAllowed, data = df)
summary(mc1)
par(mfrow=c(2,2))
plot(mc1)
...
par(mfrow=c(2,2))
plot(stepc)
residualPlots(stepc) # aquest mc1, ja seria el millor categoric.

# AFEGIM INTERACCIONS del primer d'abans
mi1 <- lm(log(price)~log(livingArea)+log(bathrooms)+poly(antiguetat,2)* (levels+event+f.sqrtPrice), data = df)
summary(mi1)
...
stepi <- lm(log(price) ~ log(livingArea) + log(bathrooms) + event + (poly(antiguetat, 2) * f.sqrtPrice), data=df)
vif(stepi, type = 'predictor')
summary(stepi)
par(mfrow=c(2,2))
plot(stepi)
residualPlots(stepi)
...
Boxplot(rstudent(stepi))
Boxplot(cooks.distance(stepi))
res1 <- rstudent(stepi)
res2 <- cooks.distance(stepi)
...
# SENSE RES DE PRICEPERSQUAREFOOT
```{r}
# CATEGORIQUES TEKA
m0 <- lm(log(price)~log(livingArea)+f.bath + f.bed + f.antig + event + parking + levels + spa + pool + hasPetsAllowed + homeType + isNewConstruction, data = df)
summary(m0)
par(mfrow = c(2,2))
plot(m0)
vif(m0, type = 'predictor')
step0 <- step(m0, k = log(nrow(df)))
vif(step0, type = 'predictor')
par(mfrow=c(2,2))
plot(step0)
residualPlots(step0)

# MILLOR es el M0, treballant amb garagespaces i antiguetat com a numeriques.

# interaccions

mix1 <- lm(log(price)~(log(livingArea))*(f.antig + event + parking + levels + pool + hasPetsAllowed + homeType + f.bed + f.bath + isNewConstruction), data = df)
summary(mix1)
par(mfrow=c(2,2))
plot(mix1)
residualPlots(mix1)
stepmix1 <- step(mix1, k=log(nrow(df)))
vif(stepmix1, type = 'predictor')
summary(stepmix1)
par(mfrow=c(2,2))
plot(stepmix1)
AIC(stepmix1, step0)

stepmix <- lm(log(price) ~ log(livingArea) * (parking + levels + hasPetsAllowed + homeType + f.bath + isNewConstruction) +event + pool + f.bed,data = df)
summary(stepmix)
par(mfrow=c(2,2))
plot(stepmix)
residualPlots(stepmix)
influencePlot(stepmix)

Boxplot(rstudent(stepmix))
Boxplot(cooks.distance(stepmix))
res1 <- rstudent(stepmix)
res2 <- cooks.distance(stepmix)

idx1 <- which(res1 < -3 | res1 > 3); length(idx1);
idx2 <- which(res2 > 0.5); length(idx2); idx2
df <- df[-c(idx1,idx2),]

# ocasionalment quan veiem el valor que es massa extrem respecte els altres -> 2598 -> id : 2684
