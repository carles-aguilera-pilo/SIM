...
res.miceB <- mice(df[,c(7,9,10,11,12)])
df_imput <- complete(res.miceB)
df$bedrooms <- df_imput$bedrooms
```
...
res.cat <- catdes(df[,c(3:22)], num.var = 20)
...
```{r}
corr_mat <- cor(df[,c(7,9,10,11,12,14)])
corr_mat
...
```{r}
res.con <- condes(df[,c(6:21)], num.var = 2)
res.con$quanti
...
```{r}
mxx1 <-  lm(sqrt(price)~sqrt(livingArea)+sqrt(bathrooms) + bedrooms, data = df)
summary(mxx1)
plot(mxx1)
residualPlots(mxx1)
...
AIC(mxx1,m2)
boxTidwell(sqrt(price)~sqrt(livingArea)+sqrt(bathrooms) + bedrooms, data = df)

mxx2 <-  lm(sqrt(price)~sqrt(livingArea)+sqrt(bathrooms) + sqrt(bedrooms), data = df)
summary(mxx2)
plot(mxx2)
residualPlots(mxx2)
...
modelv444 <-lm(log(price)~log(livingArea)+log(bathrooms)+log(bedrooms)+poly(garageSpaces,2), data = df)
summary(modelv444)
plot(modelv444)
