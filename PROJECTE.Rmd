...
par(mfrow=c(2,2))
plot(modelv44)
vif(modelv44) # SEGONS CERTS ANÀLISI D?INTERNET EL MILLOR SERIA AQUEST.

# INTERACTIONS 

modelv444 <- lm(sqrt(df$price)~log(df$livingArea)+sqrt(df$bathrooms)+log(df$bedrooms)*df$garageSpaces)
summary(modelv444)
par(mfrow=c(2,2))
plot(modelv444)
vif(modelv444)
AIC(modelv44, modelv444)

# AQUI QUE FEM PERQUE SERIA MILLOR AGAFAR EL MODELV444.

modelv4444 <- lm(sqrt(df$price)~log(df$livingArea)+sqrt(df$bathrooms)*df$garageSpaces+log(df$bedrooms))
summary(modelv4444)
par(mfrow=c(2,2))
plot(modelv4444)
vif(modelv4444)
AIC(modelv44, modelv4444)
AIC(modelv444,modelv4444) # SERIA AQUEST ULTIMA LA MILLOR ? 


modelv4444x <- lm(sqrt(df$price)~log(df$livingArea)*df$garageSpaces+sqrt(df$bathrooms)+log(df$bedrooms))
summary(modelv4444x)
par(mfrow=c(2,2))
plot(modelv4444x)
vif(modelv4444x) #COLINEARITAT.
AIC(modelv44, modelv4444x)
AIC(modelv4444, modelv4444x) # SERIA EL MILLOR.
...
# ES MILLOR NO INCLOURE EL YEARBUILT, AMB ANOVA HO HEM COMPROVAT.




model555 <- lm(sqrt(df$price)~log(df$livingArea)*df$garageSpaces+sqrt(df$bathrooms)+log(df$bedrooms)+df$yearBuilt)
summary(model555)
vif(model555) # HI HA COLINEARITAT. 

# INTERACTION

modelv5 <- lm(sqrt(df$price)~log(df$livingArea)+sqrt(df$bathrooms)+log(df$bedrooms)+df$garageSpaces*df$yearBuilt)
summary(modelv5)
vif(modelv5)
