...
ASSUMIM QUE LA DISTRIBUCIÓ DE PRICE SEGUEIX UNA DISTRIBUCIÓ NORMAL, NO OBSTANT, SI HAGUESSIM DE CANVIAR,SERIA PASSAR
DE TEST PARAMETRICS A TEST NON-PARAMETRIC. 
...
# Parametric
oneway.test(df$price~df$city, data = df) # SOME ERRORS !!!!
...
oneway.test(df$price~df$county, data = df) # Errors
...
oneway.test(df$price~df$event, data = df)
...
# PARAMETRIC
oneway.test(df$price~df$levels, data = df) # SOME ERRORS.
...
t.test(df$price~df$homeType, alternative = 'two.sided', conf.level = 0.95)
# p-value = 4.743e-09. P-VALUE < 0.05 so we CAN reject the null hypotesis.
...
# PARAMETRIC
t.test(df$price~df$pool, alternative='two.sided', conf.level = 0.95)
# P-VALUE < 0.05. PRICE IS DIFFERENT IF YOU HAVE POOL OR NOT, SO WE CAN REJECT THE NULL HYPOTESIS, LA REFUTEM LA H0.
