...
acf(df$price) # Veiem com si tenim autocorrelation, es a dir, price esta correlacionada amb els seus valors anteriors.
```
...
```{r}
model1 <- lm(df$price~df$livingArea+df$is_bankOwned+df$is_forAuction+df$event+df$city+df$yearBuilt+df$bathrooms+df$bedrooms
             +df$parking+df$garageSpaces+df$levels+df$pool+df$spa+df$isNewConstruction+df$hasPetsAllowed+df$homeType
             +df$county)
summary(model1)

model2 <- lm(df$price~df$livingArea+df$is_bankOwned+df$is_forAuction+df$event+df$yearBuilt+df$garageSpaces+df$levels+df$pool+
            df$spa+df$isNewConstruction+df$hasPetsAllowed+df$homeType)
summary(model2)

anova(model1, model2) # HERE WE HAVE THAT THE MODEL 1 IS BEST THAN THE SECOND ONE, THE SECOND ONE , MODEL 2, IS NOT AS GOOD AS MODEL 1.
```
