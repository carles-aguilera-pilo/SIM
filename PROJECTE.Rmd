...
# PROBEM SI ES MILLLOR SEGUIR UTILITZANT NUMERIQUES O CATEGORIQUES. 

# usem les discretitzades
...
AIC(mcm,mc1)
# usem les discretitzades, AIC es menor.
...
mi1 <- lm(log(price)~(log(livingArea))*(f.bath +f.antig+event+isNewConstruction+homeType+f.sqrtPrice), data = df)
summary(mi1)
...
stepi <- lm(log(price) ~ log(livingArea) * f.sqrtPrice + f.antig + event + homeType,data=df)
...
# Treballem amb categoriques perquè hem vist adalt com es millor treballar amb categoriques.
...
stepmix<-lm(log(price)~log(livingArea)*(f.antig+levels+hasPetsAllowed+f.bath+isNewConstruction)+event+parking+pool+ f.bed,data = df)
summary(stepmix)
par(mfrow=c(2,2))
plot(stepmix)
residualPlots(stepmix)
influencePlot(stepmix)
vif(stepmix, type = 'predictor')
