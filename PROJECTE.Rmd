...
df$bedrooms[which(df$bedrooms == 0)] <- NA

res.miceB <- mice(df[,c(6,8,9,10,11,13)])
...
df <- subset(df, select = -has_garage)
...
res.mout <- Moutlier(df[,c(6,8,9,10,11,13)], quantile = 0.99)
...
```{r}
corr_mat <- cor(df[,c(6,8,9,10,11,13)])
corr_mat
...
df <- df[ll,]
acf(df$price) # WE CAN SEE NOT AUTOCORRELATION, SO IT'S INDEPENDENT.
```
...
```{r}
res.con <- condes(df[,c(3:21)], num.var = 4)
res.con$quanti
...
```{r}
par(mfrow = c(1,1))
model1 <- lm(df$price~df$livingArea)
summary(model1)

plot(df$price~df$livingArea)
lines(df$livingArea, fitted.values(model1), col = 'red')
#text(df$price~df$livingArea, labels = row.names(df))
par(mfrow=c(2,2))
plot(model1)

# WE CAN SEE HERE THAT THE NORMALITY IS NOT ACHIVED. 
# ALSO THE HOMOSKEDASTIC PROPERTY IS NOT ACHIEVED, AT THE FRIST WE SEE POINTS MORE CLOSER AND IN THE MIDDLE MORE DISPERSION
# LINEARITY IS NOT ACHIEVED ALSO.

```

```{r}
cooksD <- cooks.distance(model1)
Boxplot(cooks.distance(model1))
abline(h = 4 / length(cooksD), col = "red", lty = 2)

influents <- sort(cooksD, decreasing = TRUE)
head(influents, 20)
threshold <- 4 / length(cooksD)
influential_points <- which(cooksD > threshold); length(influential_points)

```

```{r}
par(mfrow = c(1,1))
boxCox(model1) # THE VALUE FOR THE PRICE IS CLOSE TO 0.5 -> SO WE HAVE TO APPLY SQRT
modelv1 <- lm(sqrt(df$price)~df$livingArea)
summary(modelv1)
boxTidwell(df$price~df$livingArea) # Value close to 1, so we'll not apply any transformation.
```



```{r}
model2 <- lm(df$price~df$livingArea+df$bathrooms)
summary(model2)

par(mfrow=c(2,2))
plot(model2)
# We haven't still have the linearity, normality and homoskedastik properties.

boxCox(model2) # PROPER TO 0.5, let's apply sqrt()

modelv2 <- lm(sqrt(df$price)~df$livingArea+df$bathrooms)
summary(modelv2)
par(mfrow=c(2,2))
plot(modelv2)

# WE HAVE IMPROVED BUT WE STILL HAVING SOME PROBLEMS, NOW LET'S TAKE ACTION WITH THE BOXTWIDELL.

boxTidwell(sqrt(df$price)~df$livingArea+df$bathrooms)
# We obtain a lambda below 0, in that case we have to do something, in that case apply sqrt().

modelv22 <- modelv2 <- lm(sqrt(df$price)~sqrt(df$livingArea)+sqrt(df$bathrooms))
summary(modelv22)

par(mfrow=c(2,2))
plot(modelv22)

# HERE WE ACHIEVED MORE LINEARITY IN THE FIRST PLOT, AND MORE NORMALITY, ALSO WE HAVE IMPROVED A LOT THE HOMOSKEDASTICITY, BUT WE CAN IMPOROVE BETTER.
...
par(mfrow=c(1,1))
boxCox(model5) # Close to 1, so we will not make any change.
```
